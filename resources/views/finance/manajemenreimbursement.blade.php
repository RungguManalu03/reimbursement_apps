                    ...
                    <div class="col-xl-12 col-md-12 col-lg-12 ">
                        <div class="card overflow-hidden">
                            <div class="card-header">
                                <h5 class="card-title mt-2">Manajemen Reimbursement (Role Finance)</h5>
                            </div>
                            <div class="card-header bg-transparent pd-b-0 pd-t-20 bd-b-0">
                                <h4 class="card-title mg-b-10">Halaman Manajemen Reimbursement (Role Finance)</h4>
                ...
                <div class="modal-body">
                    <form action="javascript:void(0);" id="formDetailReimbursement">
                        ...
                        <div class="row g-3">
                            ...
                            <input type="text" class="form-control" style="display: none" id="id-data" name="id-data" required="required">
                            <div class="col-lg-12 d-flex justify-content-center align-items-center">
                                <div class="hstack gap-2 justify-content-end">
                                    <button type="submit" class="btn btn-primary" id="button-verifikasi">Verifikasi</button>
                                    <button type="button" class="btn btn-danger"  id="button-tolak">Tolak</button>
                                </div>
                            </div>
                        </div>
                    </form>
...
@section('js')
    ...
    <script>
        ...
        let reimbursement_datatable = $("#reimbursements-list").DataTable({
            processing: true,
            serverSide: true,
            ajax: `{{ route('list-data-reimbursement-user-finance') }}`,
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex',
                    orderable: false,
        ...
        $("#reimbursements-list").on('click', '#btn-detail-reimbursement', function() {
             ...
             $.ajax({
                url: "{{ route('find-data-reimbursement-user-finance') }}",
                data: {
                    "id": id,
                    "_token": "{{ csrf_token() }}",
                },
        ...
        $('#button-verifikasi').on('click', function() {
            var idData = $('#id-data').val();
            var url = "{{ route('verifikasi-reimbursement-user-finance') }}";
            Swal.fire({
                title: "Apakah anda yakin?",
                text: "Pastikan data yang anda verifikasi sudah benar!",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            $('#detailUserReimbursement').modal('hide');
                            if (response.success) {
                                Swal.fire({
                                    html: `<div class="mt-3"><lord-icon src="https://cdn.lordicon.com/lupuorrc.json" trigger="loop" colors="primary:#0ab39c,secondary:#405189" style="width:120px;height:120px"></lord-icon><div class="mt-4 pt-2 fs-15"><h4>Berhasil!</h4><p class="text-muted mx-4 mb-0">${response.message}</p></div></div>`,
                                    timer: 3000
                                }).then(() => {
                                    Swal.close();
                                    location.reload();
        ...
        $('#button-tolak').on('click', function() {
            var idData = $('#id-data').val();
            var url = "{{ route('tolak-reimbursement-user-finance') }}";
            Swal.fire({
                title: "Apakah anda yakin?",
                text: "Pastikan data yang anda tolak sudah benar!",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
